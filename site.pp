...
class docker {
$admintools= ['curl', 'apt-transport-https', 'ca-certificates','gnupg-agent','software-properties-common']
package { $admintools :
 ensure => present,
 }
 exec {'apt-update':
 ...
 exec {'install_docker':
 command => '/usr/bin/apt-get -y --allow-unauthenticated --force-yes install docker-ce'
 }
}
